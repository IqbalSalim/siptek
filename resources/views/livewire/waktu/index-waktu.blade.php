                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::createFromFormat('H:i:s', $row->come_start_time)->format('H:i') . ' -
                                ' . \Carbon\Carbon::createFromFormat('H:i:s', $row->come_end_time)->format('H:i') }}
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::createFromFormat('H:i:s', $row->come_time)->format('H:i') }}
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::createFromFormat('H:i:s', $row->go_start_time)->format('H:i') . ' - '
                                . \Carbon\Carbon::createFromFormat('H:i:s', $row->go_end_time)->format('H:i') }}
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::createFromFormat('H:i:s', $row->go_time)->format('H:i') }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button @click="modalEdit=true" wire:click.prevent="$emit('getTime', {{ $row->id }})"
