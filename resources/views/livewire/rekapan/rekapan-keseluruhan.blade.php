<div x-cloak x-data="{ rekapan: false }" x-on:close-rekapan="rekapan=false">
    ...
    <div id="content">
        ...
        <div class="relative overflow-x-auto border shadow-md sm:rounded-lg dark:border-gray-700">
            ...
            <div
                class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:items-end justify-between p-4 md:space-x-4 bg-white dark:bg-gray-800">
                <div class="flex flex-row  items-end justify-between md:justify-start md:space-x-4">
                    <div>
                ...
                <div x-show='rekapan' class="block text-right">
                        ...
                        <tr
                            ...
                            @for ($i = 1; $i <= $this->countDays; $i++)
                                <td class="px-6 py-4 border border-slate-600">
                                    ...
                                    {{ $row['pertanggal'][$i]['come_presence'] ? 'Jam Masuk: ' .
                                    \Carbon\Carbon::createFromFormat('H:i:s',
                                    $row['pertanggal'][$i]['come_presence'])->format('H:i') : null }}
                                    {{ $row['pertanggal'][$i]['go_presence'] ? 'Jam Pulang: ' .
                                    \Carbon\Carbon::createFromFormat('H:i:s',
                                    $row['pertanggal'][$i]['go_presence'])->format('H:i') : null }}
                                    <br>
                                    {{ $row['pertanggal'][$i]['late_minutes'] ? 'Terlambat :' .
                                    $row['pertanggal'][$i]['late_minutes'] . ' (' . $row['pertanggal'][$i]['come_code']
                                    . ')' : null }}
                                    <br>
                                    {{ $row['pertanggal'][$i]['quick_minutes'] ? 'Cepat Pulang :' .
                                    $row['pertanggal'][$i]['quick_minutes'] . ' (' . $row['pertanggal'][$i]['go_code'] .
                                    ')' : null }}
                                </td>
                                @php
                                $totalTerlambat = $row['pertanggal'][$i]['late_minutes'] ? $totalTerlambat +
                                $row['pertanggal'][$i]['late_minutes'] : $totalTerlambat;
                                $totalCepat = $row['pertanggal'][$i]['quick_minutes'] ? $totalCepat +
                                $row['pertanggal'][$i]['quick_minutes'] : $totalCepat;
                                @endphp
                                @endfor
                                @endif
                                <td class="px-6 py-4 border border-slate-600">
